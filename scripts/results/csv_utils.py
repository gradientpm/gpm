...
def extractCSVHeader(filename):
    ...
    try: 
        ...
        print("CSV file header error: " + filename)
        return []
...
def extractCSVNumberLog(filename, mul, column=0):
    ...
    try:
        ...
        print("CSV file IO (logarithm) error: " + filename)
        return []
...
def extractCSVNumber(filename, column=0):
    ...
    try:
        ...
        print("CSV file IO error: " + filename)
        return []
...
class Technique:
    def __init__(self, name, color, xCSVFile, yCSVFile, step, log, shift=0, column=0):
        ...
        if((len(self.x)/self.step) != len(self.y)):
            print("===============================================")
            print("[WARN] The CSV Files doesn't have the same size")
            self.dump()
            print("[INFO] Clip to the minimum ... :(")
