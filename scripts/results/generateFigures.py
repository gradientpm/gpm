...
def saveNPImage(imgPath,pRef,output,scale=1.0):
    ...
    print("NP Image: Min",min(pixelsHDR),"Max",max(pixelsHDR))
...
def saveFig(w,h,data,output,cmapData=None, minData=None, maxData=None):
    ...
    print("[INFO] Detected size: ",widthN,heightN, "targeted", w, h)
...
class MetricOp:
    ...
    def show(self, wk):
        ...
        if(self.exposure != 0.0):
            print("[WARN] Exposure not supported during metric op.")
        mult = 1.0#float(math.pow(2, float(self.exposure)))
        images = [wk + os.path.sep + self.img]
        errors = rgbe.fast.rmse_all_images(w,h, images, pRef, mult, None)[0]
        errorsNames = ["mse", "rmse", "mseLog", "rmseLog", "tvi", "relative", "relMSE"]
        
        print("[DISPLAY] Metric for "+self.img+" ( with "+self.ref+")")
        for i in range(len(errors)):
            print(errorsNames[i],":",errors[i])
...
class ImageOp(object):
    ...
    def loadHDR(self, wk):
        ...
        if(self.expo != 0.0 or self.gamma != 1.0):
            print("Expo:",self.expo,"Gamma:",self.gamma)
    ...
    def loadIm(self):
        ...
        rgbe.utils.copyPixeltoPIL(self.width, self.height, self.pixelsHDR, self.im)
...
class ImagePNDiffOp(ImageFalseColorOp):
    ...
    def loadHDR(self, wk):
        ...
        print("Min",min(self.pixelsHDR),"Max",max(self.pixelsHDR))
        self.maxV = 1.0
        self.minV = -1.0
