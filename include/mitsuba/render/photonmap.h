...
public:
	 ...
	 *
	 * Does no weighting/filtering/dynamic search radius reduction
	 * and simply sums over all photons. Only considers path length with
	 * a depth value greater than or equal \c minDepth and 
     * less than or equal \c maxDepth parameter.
	 * This function is used in SPPM implementation.
     */
    size_t estimateRadianceGP(const Intersection &its,
					 	      Float searchRadius, Spectrum &result, 
                              int gatherPointDepth, int minDepth, int maxDepth,
							  int currSComp) const;
