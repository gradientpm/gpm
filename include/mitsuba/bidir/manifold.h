...
public:
    ...
    /// Debug ======
    Matrix2x2 getAi(const Path &path, int b, int c);
    Matrix2x2 getB1(const Path &path, int b, int c);
    Matrix2x2 getBk(const Path &path, int b, int c);
    
    Matrix2x2 getAiB1(const Path &path, int b, int c);
    Matrix2x2 getAiBk(const Path &path, int b, int c);
    
    Float getDetAiB1(const Path &path, int b, int c);
    Float getDetAiBk(const Path &path, int b, int c);
    Float getDetAiB1(const Path &path, int b, int c, int i);

    // area ratio of the first specular vertex to the last vertex
    Float getDetDx2Dxk(const Path &path, int a, int b);

    Float naiveG(const Path &path, int b, int c);
...
protected:
    ...
    /// This stores the path initialized. Not changed during move().
    /// This could be used for faster generalized geometry term calculation.
	std::vector<SimpleVertex> m_sourceVertices; 
