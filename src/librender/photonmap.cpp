...
struct RadianceQueryGp {
  RadianceQueryGp(const Intersection &its, int gatherPointDepth, int minDepth, int maxDepth, int currSComp, Float radius)
  : its(its), gatherPointDepth(gatherPointDepth), minDepth(minDepth), maxDepth(maxDepth),
    currSampledComponent(currSComp), searchRadius(radius),
    result(0.0f), M(0) 
  {
	  bsdf = its.getBSDF();
  }
  ...
  inline void operator()(const Photon &photon) {
	  ...
	  int pathLength = gatherPointDepth + photon.getDepth();
      if (pathLength < minDepth || pathLength > maxDepth) return;

      // TODO: interaction mode (how to store prevComponentType nicely?)
...
size_t PhotonMap::estimateRadianceGP(const Intersection &its,
									 Float searchRadius, Spectrum &result, 
                                     int gatherPointDepth,
                                     int minDepth, int maxDepth,
									 int currSComp) const {
	RadianceQueryGp query(its, gatherPointDepth, minDepth, maxDepth, currSComp, searchRadius);
