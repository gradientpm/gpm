				...
				if(shifted.alive) {
					...
					if(shifted.connection_status == RAY_CONNECTED) {
						...
						if(mainVertexType == VERTEX_TYPE_DIFFUSE && mainNextVertexType == VERTEX_TYPE_DIFFUSE && shiftedVertexType == VERTEX_TYPE_DIFFUSE) {
							...
							// For solving crosses initialization on linux
							...
							// Update throughput and pdf.
							bRec.wo = tangentSpaceOutgoingDirection; // Update outgoing dir
...
void averageBuffer(const Vector2i& cropSize, Bitmap* accum, Bitmap* iter, int iterationNumber) {
  ...
  Spectrum *accumPtr =    (Spectrum *)accum->getUInt8Data();
  Spectrum *iterPtr =     (Spectrum *)iter->getUInt8Data();
      ...
      if (m_dumpIteration > 0 && currentIteration % m_dumpIteration == 0) {
		  ...
		  if(m_config.m_reconstructL2) {
			  ...
			  if(m_config.m_reconstructL1) {
			  	  develop(scene, hdrFilm, reconstructionBitmap, currentIteration, "_L2_");
			  } else {
				  develop(scene, hdrFilm, reconstructionBitmap, currentIteration, "_recons_");
			  }
		  ...
		  if(m_config.m_reconstructL1) {
			  ...
			  if(m_config.m_reconstructL2) {
			  	  develop(scene, hdrFilm, reconstructionBitmap, currentIteration, "_L1_");
			  } else {
				  develop(scene, hdrFilm, reconstructionBitmap, currentIteration, "_recons_");
			  }
          ...
          if(m_showAbsGradients) {
            ...
            // copy abs gradient values
            Spectrum *tDX = (Spectrum *) dxBitmap->getUInt8Data();
            Spectrum *tDXAbs = (Spectrum *) dxBitmapAbs->getUInt8Data();
            Spectrum *tDY = (Spectrum *) dyBitmap->getUInt8Data();
            Spectrum *tDYAbs = (Spectrum *) dyBitmapAbs->getUInt8Data();
            for (int y = 0; y < cropSize.y; ++y) {
              for (int x = 0; x < cropSize.x; ++x) {
                int pixelID = y * cropSize.x + x;
