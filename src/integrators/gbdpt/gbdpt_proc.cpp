		...
		for (int s = (int)emitterSubpath.vertexCount() - 1; s >= 0; --s) {
			...
			for (int t = maxT; t >= minT; --t) {
                ...
                if(!m_config.directTracing) {
                  ...
                  bool roughPath = false;
                  for(int localT = t; localT >= 2; --localT) {
                      const Intersection& itsLocal = sensorSubpath[0].vertex(localT)->getIntersection();
                      const BSDF* localBSDF = itsLocal.getBSDF();
                      if(localBSDF == 0) continue;
                      for(int localCompBSDF = 0; localCompBSDF < localBSDF->getComponentCount(); localCompBSDF++) {
                          roughPath |= localBSDF->getRoughness(itsLocal, localCompBSDF) > 0.05;
                      }
                  }
                  for(int localS = s; localS >= 2; --localS) {
                      const Intersection& itsLocal = emitterSubpath.vertex(localS)->getIntersection();
                      const BSDF* localBSDF = itsLocal.getBSDF();
                      if(localBSDF == 0) continue;
                      for(int localCompBSDF = 0; localCompBSDF < localBSDF->getComponentCount(); localCompBSDF++) {
                          roughPath |= localBSDF->getRoughness(itsLocal, localCompBSDF) > 0.05;
...
#if 0
				int diffuseF = -1;
				int nextDiffuseIsDiffuse = -1;
				for(int _t = 2; _t < t; _t++) {
					int bsdfType = sensorSubpath[0].vertex(t)->getIntersection().getBSDF()->getType();
					bool deltaSurf = !(bsdfType & BSDF::EDiffuseReflection) && !(bsdfType & BSDF::EGlossyReflection);
					if(!deltaSurf && diffuseF == -1) {
						diffuseF = _t;
					} else if (diffuseF != -1 && diffuseF + 1 == _t) {
						nextDiffuseIsDiffuse = deltaSurf;
					}
				}
				if(diffuseF == -1 || nextDiffuseIsDiffuse == -1) {
					if(diffuseF != -1) {
						diffuseF = s + 1; // Hack
					}
					// Still to work around light path
					for(int _s = s; _s > 1; _s--) {
						int bsdfType = emitterSubpath.vertex(_s)->getIntersection().getBSDF()->getType();
						bool deltaSurf = !(bsdfType & BSDF::EDiffuseReflection) && !(bsdfType & BSDF::EGlossyReflection);
						if(!deltaSurf && diffuseF == -1) {
							diffuseF = _s;
						} else if (diffuseF != -1 && diffuseF - 1 == _s) {
							nextDiffuseIsDiffuse = deltaSurf;
						}
					}
				}
				...
				} else if(nextDiffuseIsDiffuse != -1) {
					if(nextDiffuseIsDiffuse) {
						//continue;
					} else {
						continue;
					}
