...
bool SpecularManifold::move(const Point &target, const Normal &n) {
	...
	while (m_iterations < m_maxIterations) {
        ...
        /* GPM: Save original vertex. Only run once. */
        if (m_sourceVertices.size() == 0) {
            m_sourceVertices.assign(m_vertices.begin(), m_vertices.end());
        }
 ...
 * If the chain is pure specular, it returns | dx_b+1 / dx_s | 
 * to facilitate computing Jacobian for shift mapping in gradient-domain rendering.
 */
Float SpecularManifold::det(const Path &path, int b, int c) {
    ...
    if (pb->isMediumInteraction()) {
        vb.n = Vector(path.edge(b < c ? (c - 1) : c)->d);
    }
    else {
        vb.n = pb->getShadingNormal();
    }
    ...
    if (pc->isMediumInteraction()) {
        vc.n = Vector(path.edge(b < c ? (c - 1) : c)->d);
    }
    else {
        vc.n = pc->getShadingNormal();
    }
    coordinateSystem(vc.n, vc.dpdu, vc.dpdv);
    ...
    // For pure specular, the Jacobian is simply | dx_b+1 / dx_s |
    if (nSpecular == n - 1) {
        return std::abs(m_vertices[1].Tp.det());
    }

    // Fill (n-1) rows of the n x n block matrix
    // Each row corresponds to a half-vector constraint 
    //    c(x1, x2, x3) = 0 where x2 the current vertex
    // Each block is the derivative of the half vector w.r.t. the vertex position
    for (int j = 0; j < n - 1; ++j) {
		...
		//if (j+1 <= n) // no need to check because condition is always met
        {
            // dOk / dx_{k+1}
			A(2*i,   2*(j+1))   = m_vertices[j+1].c(0,0);
			A(2*i,   2*(j+1)+1) = m_vertices[j+1].c(0,1);
    ...
    // Fill the last row
    {
        ...
        // the following blocks are not necessary? 
        for (int j = 0; j < n-1; ++j) {
        //for (int j = n-2; j < n-1; ++j) {   // only adjacent blocks due to natural constraint?
            // dx_s / dx_{j+1} = Tp^-1
            Matrix2x2 U;
...
Matrix2x2 SpecularManifold::getAi(const Path &path, int b, int c) {
    int k = path.length();

    int step = c > b ? 1 : -1, nGlossy = 0, nSpecular = 0;

    // only middle vertices?
    ...
    // assume only 1 specular vertex for debugging
    SAssert(nSpecular == 1);
    ...
    int c_idx = b_idx + c - b;
    SimpleVertex &vc = m_vertices[c_idx];
    const PathVertex *pc = path.vertex(c);
    ...
    // Test
    //Float d = m_vertices[1].Tp.det();
    //SLog(EInfo, "Determinant of Tp: %f", d);
...
Matrix2x2 SpecularManifold::getBk(const Path &path, int b, int c) {
    int k = path.length();
    ...
    // have to initialize dpdu and dpdv of the last vertex
    // to properly extract Bk.
    int c_idx = b_idx + c - b;
    SimpleVertex &vc = m_vertices[c_idx];
    const PathVertex *pc = path.vertex(c);
    ...
    //return m_vertices[c - b - 1].c;
    return m_vertices[(int)m_vertices.size() - 2].c;
}

Matrix2x2 SpecularManifold::getB1(const Path &path, int b, int c) {
    int k = path.length();
    ...
    return m_vertices[1].a;

}

inline Float geometryTerm(const Point &p, const Vector &pn, const Point &q, const Vector &qn) {
    Vector pq = q - p;
    return absDot(pn, normalize(pq)) * absDot(qn, normalize(pq)) / (pq.lengthSquared());
}
/**
 * Use the matrix formula to compute generalized geometry term. 
 * For verification with G() implemented in Mitsuba. 
 */
Float SpecularManifold::naiveG(const Path &path, int b, int c) {
    Matrix2x2 Ai = this->getAi(path, b, c);
    Matrix2x2 Bk = this->getBk(path, b, c);
    Float detAiBk = (-Ai * Bk).det();
    Float G12 = geometryTerm(path.vertex(b)->getPosition(), path.vertex(b)->getGeometricNormal(),
                             path.vertex(b + 1)->getPosition(), path.vertex(b + 1)->getGeometricNormal());
    Float G1k = G12 * detAiBk;
    return G1k;
}

Matrix2x2 SpecularManifold::getAiB1(const Path &path, int b, int c) {
    Matrix2x2 Ai = getAi(path, b, c);
    Matrix2x2 B1 = getB1(path, b, c);
    return -(Ai * B1);
}

Matrix2x2 SpecularManifold::getAiBk(const Path &path, int b, int c) {
    Matrix2x2 Ai = getAi(path, b, c);
    Matrix2x2 Bk = getBk(path, b, c);
    return -(Ai * Bk);
}

Float SpecularManifold::getDetAiB1(const Path &path, int b, int c) {
    Matrix2x2 Ai = getAi(path, b, c);
    Matrix2x2 B1 = getB1(path, b, c);
    return -(Ai * B1).det();
}

Float SpecularManifold::getDetAiB1(const Path &path, int b, int c, int dummy) {
    bool success = init(path, b, c);
    BDAssert(success);
    ...
    /* Similarly, find the tangent space with respect to translation of the first vertex. */
    
    // Reference solution using full matrix inverse, very slow but should be correct
    ...
    Matrix2x2 B1 = m_vertices[1].a;
    // Only compute for vertices in the middle
    Float det = 1.0f;
    for (int i = 1; i <= N; ++i) 
    {
        Matrix2x2 K;
...
Float SpecularManifold::getDetAiBk(const Path &path, int b, int c) {
    Matrix2x2 Ai = getAi(path, b, c);
    Matrix2x2 Bk = getBk(path, b, c);
    return -(Ai * Bk).det();
}

Float SpecularManifold::getDetDx2Dxk(const Path &path, int a, int b) {
    // derive from G() function 
	...
	Float result = cross(m_vertices[1].map(1, 0), m_vertices[1].map(0, 1)).length();
    return result;
}
