...
bool ManifoldPerturbation::manifoldWalkGPM(const Path &source, Path &proposal, int step, int b, int c)
{
    ...
    if (!m_manifold->update(proposal, b, c)){
        ++statsMWFailUpdate;
        return false;
    }
    if (!m_manifold->move(vb_old->getPosition(), n)) {   // Reversible check
