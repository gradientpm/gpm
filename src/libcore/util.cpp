...
void coordinateSystemCoherent(const Vector &a, Vector &b, Vector &c) {
	...
	if(a.z < -0.9999999f) // Handle the singularity
	{
		b = Vector( 0.0f , -1.0f , 0.0f );
		c = Vector( -1.0f , 0.0f , 0.0f );
        return ;
	}
	const Float _a = 1.0f /(1.0f + a.z );
	const Float _b = -a.x*a.y*_a ;
	b = Vector(1.0f - a.x*a.x*_a , _b , -a.x );
	c = Vector(_b , 1.0f - a.y*a.y*_a , -a.y );
